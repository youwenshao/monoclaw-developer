...
如果 `bundle-inputs/vendor/models/gemma-4-e4b/gemma-4-E4B-it-Q4_K_M.gguf` 存在，组装器会在 `model-packs/gemma-4-e4b/` 创建一个可选的并排 sidecar。该模型包不是核心 `dist/hatch-manifest.json` 的一部分；它有自己的 `model-pack-manifest.json`，并通过 `dist/install-gemma-model.sh` 安装。
...
`install.sh` 会生成到预置包中，并调用 `bin/hatch --apply --bundle-root <dist> install`，然后在安装时启用 Mona 工具时运行 `install-mona-tools.sh`（除非 `HATCH_INSTALL_MONA_TOOLS=0`），并在 Gemma 模型包存在时运行 `install-gemma-model.sh`（除非 `HATCH_INSTALL_GEMMA_MODEL=0`）。当随身碟上有模型包时，LM Studio 必须已安装（否则在 `HATCH_INSTALL_STRICT=1` 下会中止）。
...
可选的本地推理就绪状态会使用 `hatch verify-local-inference` 单独检查。可选的 Gemma 模型包会使用 `hatch --model-pack-root <pack> verify-model-pack` 验证，并使用 `hatch --model-pack-root <pack> install-model` 或生成的 `dist/install-gemma-model.sh` 包装器部署。运行 `install-model` 前必须已安装 LM Studio。Hatch 将聊天 GGUF 与 mmproj 复制到 `~/.lmstudio/models/lmstudio-community/gemma-4-E4B-it-GGUF/`，供 LM Studio 自动发现。
