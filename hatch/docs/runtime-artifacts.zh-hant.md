...
如果 `bundle-inputs/vendor/models/gemma-4-e4b/gemma-4-E4B-it-Q4_K_M.gguf` 存在，組裝器會在 `model-packs/gemma-4-e4b/` 創建一個可選的並排 sidecar。該模型包不是核心 `dist/hatch-manifest.json` 的一部分；它有自己的 `model-pack-manifest.json`，並透過 `dist/install-gemma-model.sh` 安裝。
...
可選的本地推理就緒狀態會使用 `hatch verify-local-inference` 單獨檢查。可選的 Gemma 模型包會使用 `hatch --model-pack-root <pack> verify-model-pack` 驗證，並使用 `hatch --model-pack-root <pack> install-model` 或生成的 `dist/install-gemma-model.sh` 包裝器佈署。執行 `install-model` 前必須已安裝 LM Studio。Hatch 會將聊天 GGUF 與 mmproj 複製到 `~/.lmstudio/models/lmstudio-community/gemma-4-E4B-it-GGUF/`，供 LM Studio 自動發現。
